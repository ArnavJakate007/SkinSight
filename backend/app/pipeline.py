...
ZONE_COLORS = {
    "forehead": (200, 168, 0),
    "left_cheek": (200, 168, 0),
    "right_cheek": (200, 168, 0),
    "nose": (200, 168, 0),
    "chin_jawline": (200, 168, 0),
}

# BGR values mapped from requested UI overlay palette:
# face mesh / zone labels: #00A8C8, lesion boxes: #00C9A0 / #FF6B00
MESH_GRID_COLOR = (200, 168, 0)
LESION_BOX_PRIMARY_COLOR = (160, 201, 0)
LESION_BOX_ALERT_COLOR = (0, 107, 255)
...
def _draw_overlay(
    ...
    x1, y1, x2, y2 = face_bbox
    grid_layer = np.zeros_like(output)

    # Zone outlines
    if zone_hulls:
        for zone_name, hull in zone_hulls.items():
            color = ZONE_COLORS[zone_name]
            cv2.polylines(grid_layer, [hull], isClosed=True, color=color, thickness=1)
            M = cv2.moments(hull)
            if M["m00"] != 0:
                lx = int(M["m10"] / M["m00"])
                ly = int(M["m01"] / M["m00"])
        ...
        for zone_name, (zx1, zy1, zx2, zy2) in bbox_zones.items():
            color = ZONE_COLORS[zone_name]
            cv2.rectangle(grid_layer, (zx1, zy1), (zx2, zy2), color, 1)
            cv2.putText(
                output,
                zone_name.replace("_", " "),
                (zx1, max(18, zy1 - 6)),
    ...
    if np.any(grid_layer):
        output = cv2.addWeighted(output, 1.0, grid_layer, 0.38, 0.0)

    # Lesion bounding boxes — confidence-based colour coding
    # High confidence or inflammatory lesions use orange, others use mint.
    for idx, lesion in enumerate(lesions, start=1):
        is_alert = lesion.confidence >= 0.62 or lesion.acne_type in {
            "papule",
            "pustule",
            "nodule_cyst",
        }
        box_color = LESION_BOX_ALERT_COLOR if is_alert else LESION_BOX_PRIMARY_COLOR
        cv2.rectangle(
            output,
            (lesion.x, lesion.y),
    ...
    # Face bounding box
    cv2.rectangle(output, (x1, y1), (x2, y2), MESH_GRID_COLOR, 2)
    return output
